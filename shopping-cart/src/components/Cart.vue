  ...
  <div id="cart" class="cart col-md-offset-2 col-sm-offset-2 col-md-8 col-sm-8">
      <div class="panel panel-danger" v-if="cart.length">
          ...
          <div class="panel-body cart-detail">
              <ul>
                  <li v-for="item in cart">
                      <span class="pomergange glyphicon glyphicon-remove-circle" @click="removeItem(item)"></span>
          ...
          <div class="panel-footer">
              <button class="btn btn-danger btn-blcok">结算<span class="glyphicon glyphicon-circle-arrow-right"></span></button>
          </div>
      </div>
      <div class="cart-empty" v-else>
          <div id="content">
              <span class="pomegrange"><strong>购物车空空如也</strong></span>
          </div>
      </div>
  </div>
</template>
